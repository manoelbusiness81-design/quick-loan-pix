...
function CoefficientsPage() {
  ...
  const [form, setForm] = useState({ bank: "", prazo: "", taxa: "", coeficiente: "" });
  const [saving, setSaving] = useState(false);

  const load = () => supabase.from("coefficients").select("*").order("bank").order("prazo")
    .then(({ data }) => setList((data as Coef[]) ?? []));

  useEffect(() => { if (user) load(); }, [user]);

  const openNew = () => { setEditing(null); setForm({ bank: "", prazo: "", taxa: "", coeficiente: "" }); setOpen(true); };
  const openEdit = (c: Coef) => {
    setEditing(c);
    setForm({ bank: c.bank, prazo: String(c.prazo), taxa: String(c.taxa).replace(".", ","), coeficiente: String(c.coeficiente).replace(".", ",") });
    setOpen(true);
  };
  ...
  const save = async () => {
    ...
    const { error } = editing
      ? await supabase.from("coefficients").update(payload).eq("id", editing.id)
      : await supabase.from("coefficients").insert(payload);
    setSaving(false);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Cadastre suas tabelas de coeficientes por banco, prazo e taxa.</p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b border-border text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                <tr><th className="px-5 py-3 text-left">Banco</th><th className="px-5 py-3 text-left">Prazo</th><th className="px-5 py-3 text-left">Taxa</th><th className="px-5 py-3 text-left">Coeficiente</th><th className="px-5 py-3 text-right">Ações</th></tr>
              </thead>
              <tbody>
                {list.map((c) => (
                  <tr key={c.id} className="border-b border-border/60 last:border-0">
                    ...
                    <td className="px-5 py-3 font-display font-semibold tabular-nums text-brand">{Number(c.coeficiente).toFixed(6)}</td>
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div><Label>Coeficiente</Label><Input value={form.coeficiente} onChange={(e) => setForm({ ...form, coeficiente: e.target.value })} className="h-11" placeholder="0,021340" inputMode="decimal" /></div>
            </div>
          </div>
          <DialogFooter>
